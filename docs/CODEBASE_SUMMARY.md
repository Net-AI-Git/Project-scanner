# Scan API — Codebase Summary

This document describes the Scan API codebase: what it does, how orchestration works, the role of each file, the main algorithms, what was added versus existing, and a detailed account of every external library and its role in each file.

---

## 1. Overview

**Scan API** is a service that accepts a public GitHub repository URL and runs a **security vulnerability scan**. It fetches the repository’s file list and contents, builds a filtered context, then runs a LangGraph workflow: planner, orchestrator, workers (LLM-based per-file scan), md_writer, synthesizer. The API returns the path to the generated Markdown report.

Orchestration is implemented with **LangGraph**: **fetch → process → planner → orchestrator → workers → md_writer → synthesizer**. Shared state is `ScanState` (TypedDict). The API layer builds initial state, invokes `get_scan_graph().ainvoke(initial_state)`, and returns either an error response or `report_path`.
...
**Entry point:** `summary_api/api/main.py` — `POST /scan` builds `initial_state` (e.g. `correlation_id`, `github_url`, `scan_reports_dir`, `md_queue`), then calls `get_scan_graph().ainvoke(initial_state)`.

**Graph:** `summary_api/workflows/graph.py` — Builds a `StateGraph(ScanState)` with fetch_node, process_node (from `nodes.py`), and scan-specific nodes from `scan_nodes.py`.

**State:** `summary_api/workflows/state.py` — `ScanState` (TypedDict) holds input, outputs (`files`, `context`, `strategic_plan`, `sections`, `worker_results`, `report_path`, `result`), and error fields.

**Nodes:** `summary_api/workflows/nodes.py` — `make_fetch_node`, `make_process_node` (shared). `summary_api/workflows/scan_nodes.py` — planner, orchestrator, workers, md_writer, synthesizer.

**Back to API:** After `ainvoke`, `main.py` checks `final_state` for `error_response`; otherwise returns JSON with `report_path`.
...
| File | Role |
|------|------|
| `summary_api/api/main.py` | FastAPI app: lifespan (HTTP client, logging, LangSmith env), `POST /scan` (build state, invoke scan graph, return report_path), health and root routes, validation exception handler. |

### Workflows (orchestration)

| File | Role |
|------|------|
| `summary_api/workflows/graph.py` | Builds and compiles the scan LangGraph: `StateGraph(ScanState)`, fetch_node, process_node, planner, orchestrator, workers, md_writer, synthesizer. |
| `summary_api/workflows/state.py` | Defines `ScanState` (TypedDict) — all fields passed between nodes. |
| `summary_api/workflows/nodes.py` | `make_fetch_node`, `make_process_node` — shared by scan; audit, DLQ, error mapping. |
| `summary_api/workflows/scan_nodes.py` | Scan-specific nodes: planner, orchestrator, workers, md_writer, synthesizer. |

### Contracts (swappable interfaces)

| File | Role |
|------|------|
| `summary_api/contracts/interfaces.py` | ABCs: `RepoFetcher`, `ContextBuilder`, `VulnerabilityScanner`, `ReportSynthesizer`. |
...
| `summary_api/services/repo_processor.py` | `RepoContextBuilder` and `process_repo_files`: filtering, prioritization, directory tree, truncation to `max_chars`. |
| `summary_api/services/vulnerability_scanner.py` | `PydanticAIVulnerabilityScanner` — implements `VulnerabilityScanner` (per-file scan via LLM). |
| `summary_api/services/report_synthesizer.py` | `DefaultReportSynthesizer` — implements `ReportSynthesizer`. |

### Core and infrastructure

| File | Role |
|------|------|
| `summary_api/core/config.py` | Settings (pydantic-settings): NEBIUS_*, GITHUB_*, paths, limits; optional LANGCHAIN_* (LangSmith) for tracing and metrics; `get_settings`, `get_env_file_path`. |
...
| File | Role |
|------|------|
| `summary_api/models/schemas.py` | Pydantic: `ScanRequest`, `ErrorResponse`, `Finding`, `SectionFindings`, `Section`, `StrategicPlan`, etc. |
| `summary_api/clients/llm_client.py` | `LLMClientError` and `_is_llm_transient` — shared by scan and other services; no direct Nebius calls in this module. |

### Prompts and tools

| File | Role |
|------|------|
| `summary_api/prompts/planner_system_scan.j2`, `system_scan.j2`, etc. | Prompts for scan workflow (planner, vulnerability scanner). |
...
2. **Process:** Filter (e.g. node_modules, lock files, minified); sort by priority; build context; truncate to `max_context_chars`.
3. **Scan:** Planner produces strategic plan; orchestrator splits work into sections; workers run LLM-based vulnerability scan per file; md_writer writes Markdown report; synthesizer aggregates result.
...
- **contextlib.asynccontextmanager**: Decorator for `_lifespan` as an async context manager: setup (logging, HTTP client, LangSmith env vars from Settings) on startup, `yield`, and close of the client in `finally` on shutdown. When LangSmith is enabled, each graph `ainvoke` is called with run config metadata containing `correlation_id` so traces are correlated in LangSmith.
...
|---------|----------------------|
| fastapi | Web framework for the Scan API (routes, lifespan, validation). |
